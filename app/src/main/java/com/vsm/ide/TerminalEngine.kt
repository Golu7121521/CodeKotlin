 ...
 */
class TerminalEngine(context: Context) {

    private val prefs: SharedPreferences =
        context.getSharedPreferences("vsc_prefs", Context.MODE_PRIVATE)
    ...
    fun helpText(): String = """
        Available commands:
          ...
          clone <owner/repo>                     Clone a repo into $WORKSPACE
    ...
    """.trimIndent().replace("\$WORKSPACE", StorageUtil.WORKSPACE_PATH)
    ...
    fun clone(ownerRepo: String): String {
        ...
        val destDir = File(StorageUtil.workspaceDir(), destName)
    ...
    fun pushFile(ownerRepo: String, relativePath: String, message: String): String {
        requireToken()
        val destName = ownerRepo.replace("/", "_")
        val projectDir = File(StorageUtil.workspaceDir(), destName)
    ...
    companion object {
        const val WORKSPACE = StorageUtil.WORKSPACE_PATH
    }
}
